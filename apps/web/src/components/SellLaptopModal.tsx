...
export default function SellLaptopModal({ isOpen, onClose }: SellLaptopModalProps) {
  ...
  const handleClose = () => {
    ...
    setTimeout(() => {
      ...
      setModelName("");
      setConditions({ screenDamaged: false, keyboardIssues: false, batteryIssue: false });
      setImageBase64(null);
  ...
  // UPDATED: Async function to push laptop data to Firebase
  const handleFinalSubmit = async (e: React.FormEvent) => {
    ...
    if (result.success) {
      alert(`Success! Order ID: ${result.orderId}. Your laptop is registered in the system.`);
      handleClose();
    } else {
      ...
      <div className="max-w-[780px] mx-auto px-6 pt-7.5 pb-24">
        {step === 1 ? (
          <div className="animate-[fade-in-up_0.4s_ease-out]">
            <h2 className="text-[1.3rem] text-[#1E1B29] font-space font-bold mb-4.5">Select Laptop Brand</h2>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3 mb-7.5">
          ...
          <div className="animate-[fade-in-up_0.3s_ease-out]">
            ...
            <h2 className="text-[1.3rem] text-[#1E1B29] font-space font-bold mb-1.5">Assess your {selectedBrand} Laptop</h2>
            ...
            <form onSubmit={handleFinalSubmit} className="space-y-6 mt-6">
              ...
              <div>
                <label className="block text-[0.85rem] font-semibold text-[#1E1B29] mb-3">Does the laptop have any of these issues?</label>
                <div className="space-y-3">
                  <label className="flex items-center gap-3 p-4 border-[1.5px] border-[#E3D9F9] rounded-xl cursor-pointer hover:bg-[#FAF7FF]">
                    ...
                    <span className="text-[#1E1B29] text-[0.95rem]">Screen is damaged or has dead pixels</span>
                  </label>
                  <label className="flex items-center gap-3 p-4 border-[1.5px] border-[#E3D9F9] rounded-xl cursor-pointer hover:bg-[#FAF7FF]">
                    ...
                    <span className="text-[#1E1B29] text-[0.95rem]">Keyboard or trackpad not working properly</span>
                  </label>
                </div>
              </div>

              <div>
                <label className="block text-[0.85rem] font-semibold text-[#1E1B29] mb-2">Upload Laptop Photo (Optional)</label>
                <div className="border-2 border-dashed border-[#E3D9F9] rounded-xl p-6 text-center hover:bg-[#FAF7FF] transition-colors relative">
                  ...
                  {imageBase64 ? (
                    ...
                    <div className="text-[#6E6683] text-[0.9rem]">
                      <span className="text-[#7C3AED] font-semibold">Tap to upload</span> or snap a picture of your laptop
                    </div>
                  )}
                </div>
              </div>
